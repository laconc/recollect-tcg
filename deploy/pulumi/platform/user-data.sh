...
# user-data — gzip-COMPRESSED (main.go) so it fits EC2's 16 KiB user_data cap; cloud-init on AL2023
...
# service by anything ON the box — which is why IMDSv2 is REQUIRED with hop-limit 1 (main.go), so
